...
export async function openMuster(input: { squadId: string; dates: string[]; times: string[]; note?: string }) {
  const res = await musters.openMuster(await createClient(), input);
  if (res.ok) rev();
  return res;
}

export async function respondMuster(musterId: string, availableDates: string[]) {
  const res = await musters.respondMuster(await createClient(), musterId, availableDates);
  if (res.ok) rev();
  return res;
}
