...
import type { GameRequestStatus, Profile } from "@/lib/domain";
import type { Db, Result } from "./types";

// Game-request commands — the pure write logic behind app/actions/requests.ts.
...
export async function setGameRequestStatus(
  ...
  const { error } = await db.from("game_requests").update({ status }).eq("id", id);
  if (error) return { ok: false, error: "Couldn't update the request." };
  return { ok: true };
}

// The filer withdraws their own request; the CO can clear any.
export async function deleteGameRequest(db: Db, id: string): Promise<Result> {
  const { error } = await db.from("game_requests").delete().eq("id", id);
