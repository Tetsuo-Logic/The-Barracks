...
**Results model — forward design (Headquarters + AI territory).** A game *night* is not one game: it's many **matches** (20 COD games, a FIFA best-of-3, 18 golf holes). So the ranking unit is a **match**, not a night — "12 played, 10 wins" = 12 matches. The model grows a layer:

```
Fixture / event (scheduled) → Match / round → Result (per player)
```

A fixture holds N matches; each match records who won; rankings count **match wins** — no forced "overall night winner". A night can **mix games**, so **each match first selects its game (dropdown) → renders that game's scoreboard template** for entry/capture. Recording 20 matches by hand is impractical — which is the whole point of **AI card-capture**: players grab their end-of-game cards, AI reads each into `results` as *unconfirmed* (the `metrics` + `confirmed` fields exist for exactly this), a human confirms, the table updates. This lives on **Headquarters** (heavy multi-card capture + review); mobile just shows the finished league table. Phase 4b (finishing-order entry) is the degenerate case — fixture = one match — and extends **additively** (new `matches` table; `results` re-keys onto the match), not a rewrite.
...
- **Next:** (product) get real players signed up to populate rankings; (build) the Match layer + AI card-capture belong to **Headquarters** — choose the next phase when ready.
