...
- **2026-08-11 — Sq-3b built (awaiting `0036`).** Made Captain-as-CO real at the DB: `can_command(event)` = CO **or** the squad's Captain **or** the event's acting Captain; `start_operation` / `close_operation` become gated functions (were direct group-admin-only updates that would have denied a Captain), `set_attendance` re-gated on `can_command`, and `set_acting_captain(event, player)` lets the real Captain/CO name a stand-in for one night. Room UI: CO gets an **Acting Captain** picker (squad ops only); others see who's leading. ⚠️ `0036` is a **hard dependency of the code** (new RPCs) — run it on staging → prod **before** deploying. Code sits on `experiments`; build/tsc green.
- **Next:** run `0036` (staging → prod), then merge Sq-3b to main. Then Sq-4 (squad pages + home/calendar filter). Battles remain the big cross-group / Headquarters future.
