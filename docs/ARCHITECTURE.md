...
- **2026-08-12 — Headquarters experiment (branch `headquarters`, not merged).** First full widescreen build of §0's second surface, deliberately exploratory: *see the bigger product, then cut it back*. **It is one platform, not a second app** — HQ lives at `app/hq/*` in the same Next app and consumes the same auth, Supabase clients, domain types and `lib/data/queries`; it re-implements no business logic. A host-guarded rewrite in `middleware.ts` makes `localhost:3001/` land on `/hq` while `:3000` (and every deployed origin) is untouched, so the PWA is unaffected. `app/hq/hq.css` + `components/hq/Kit.tsx` are the HQ design system; HQ pins the dark palette so the app's light/dim themes can't break its fixed chrome. **The prototype boundary is `lib/hq/future/`** — every not-yet-built system (battles, network, presence, voice, Discord, Barracks Link, elections, commendations, leagues) lives there behind a typed adapter with a README naming what would replace it, and the UI marks those surfaces `<Proto />`. Nothing outside that folder invents domain data, and the mobile app never imports it. Kept off `main` because `main` auto-deploys.
